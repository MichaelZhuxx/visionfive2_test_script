...
pss_reg=$(phytool read eth0/0/0x11)
echo "PHY0 Specific Status: $pss_reg"
let "speedmode=pss_reg&0xc000"
let "speedmode=speedmode>>14"
echo "PHY0 speedmode: $speedmode"
if [[ $speedmode == 0 ]]
then
	speed_mode=10M
elif [[ $speedmode == 1 ]]
...
echo "PHY0 speed_mode: $speed_mode"

let "linkstatus=pss_reg&0x0400"
let "linkstatus=linkstatus>>10"
echo "PHY0 linkstatus: $linkstatus"

if [[ $ping_over != 0 ]] && [[ $speedmode == 2 ]] && [[ $linkstatus == 1 ]]
then
	echo "ETH0 PING PASS"
	echo "ETH0 PING:      PASS $speed_mode" >> test_result.log
	echo "PASS $speed_mode" > $log_file
else
	echo "ETH0 PING FAIL"
	echo "ETH0 PING:      FAIL $speed_mode" >> test_result.log
